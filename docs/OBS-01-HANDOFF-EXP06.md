...
> **2026-09-12 更新（分支 `backend/exp06-priority-fix`）**：修掉了优先回助的一个实现错误——
> 历史受助量曾意外成为本轮回助的**额度上限**。修复后同一批预置案例的数字变了
> （例：第 124 年那笔回助由 6457 kcal 变为 1,011,984 kcal），引擎身份也换了新的 sha256。
> 下面的示例**全部是修复后重取的**。字段结构没有任何变化，仍是 obs-1.5，前端无需改动。
...
| `preset-exp06-recip1000` | **开启**优先回助（`RECIP_M=1000`） | 首次援助 **第 83 年**；回助 3 笔，首次 **第 124 年**（1,011,984 kcal）；**分配改变 2 次，首次第 125 年** |
...
```jsonc
{
  ...
  "engine": "exp06",
  "engine_label": "EXP-06 援助记忆与优先回助",
  "engine_sha256": "43c9338551a23d6d7065f657394a25d5be041e63724ab12a23350bde53e5d186",
  "baseline_commit": "本轮实现（待审）",               // 修复后的新引擎身份
  "model_run_id": "a14b919b2fda95f1a54a2a107a0f05c0",   // 世界身份（种子+初始禀赋+参数指纹）
...
```jsonc
{
  ...
  "cell": 0,
  "kcal": 1011984,                  // 修复前这里被历史受助量压成 6457
  "person_years": 1.3863,           // = kcal / 730000，换算值
  "text": "回助 · 群体-0C98E8 向 群体-69066D 援助了 1011984 kcal（1.39 人年口粮），地点在第 0 号格（优先回助阶段：供给方记得对方帮过自己）",
  "source": "模型日志 st['aid_log']",
  "unrecorded": "动机、路线与因果关系未记录：模型里没有这些量，不要为动画补编。",
  "basis": {                        // obs-1.5 新增：**可核实的依据**
    "why": "供给方的援助记忆里有接收方，且该记忆只由实际转移累加",
    "remembered_kcal": 6457,        // 援助前记住的累计量（**只是排序依据，不是额度上限**）
...
```jsonc
{
  "cell": 0,
  "changed": true,
  "with_totals":    [{"donor": "9077…", "receiver": "7567…", "kcal": 271377}],
...
读法：开启优先后，**旧伙伴 7567 拿到全部 271,377 kcal**；关闭时则是 119,327 给旧伙伴、
152,050 给陌生群体。同一年第 6 号格的那条 `changed: false` —— 开/关结果完全一样，如实标出来。
...
```jsonc
{
  ...
  "kcal": 271377,             // 当年优先阶段实际转移（kcal）
  "transfers": 1,             // 当年优先阶段笔数
  "changed": 1,               // 当年"分配确实被改变"的格数
  "repay_kcal": 271377, "repay_transfers": 1,     // 当年回助（含碰巧）
  "cum_kcal": 1283361, "cum_changed": 1, "cum_repay_transfers": 2,   // 累计
