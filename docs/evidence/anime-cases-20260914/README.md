...
→ 通过 123 / 失败 0（共 123 项），退出码 0

# 可选：手上有真正落过盘的观察台数据目录时，再多对一层
python3 docs/evidence/anime-cases-20260914/verify-cases.py --data-dir /路径/到/observer-data
→ 通过 131 / 失败 0（共 131 项），退出码 0
```

**不依赖任何临时目录。** 脚本的数据有两个来源，都在仓库里：

1. 本目录里**已提交的真实 API 响应**（`run{A,B,C,D}-meta.json`、`runA-year-*.json`、
   `runB/C/D-year-*.json`、`bandA-*.json`、`relationsA-y295.json`）；
2. 用仓库里的引擎与 `observer/adapter.py`，**按 `real-cases.json` 里记着的参数就地重算
   整段历史**（参数从证据文件读，不写死；只读引擎，不起服务、不落盘）。

脚本先证明「就地重算 == 已提交的 API 响应」—— 四条运行、17 个被引用的年度记录
**逐字段相同**，重算用的引擎 sha256 也与记录里的一致；然后才用重算出来的整段历史去查
需要全年数据的断言（301 年逐年累计、6969 条事件去重、弃耕轨迹、C 与 B 的 301 年逐年对照）。
已提交的卷宗与关系网也不是摆设：它们的 `sizes` / 血缘 / 每条边的 `kcal` 与 `event_ids`
都要逐条对回整段历史。

给了 `--data-dir` 就改用那份真正落过盘的 `years.jsonl`，并额外断言它与重算逐年相同
（本机实测：四条运行各 301 年，0 处不同）。

最后一组是**跨实例回放**：上一轮那条 12 年样例运行（`29d5e925003b`，另一个数据目录、
另一个 `run_id`）与 A 的第 12 年逐字段相同，包括状态摘要 —— 脚本会先断言两者参数一致，
再比内容，所以这条比较的输入也是可重现的。
...
## R1 返工（2026-09-14）：复核脚本原来只能在产生它的那台机器上跑

验收在独立工作树里执行 `python3 docs/evidence/anime-cases-20260914/verify-cases.py`，
退出码 1：

```
FileNotFoundError: [Errno 2] No such file or directory:
  '/tmp/chronicle-exp07-20260913/anime/data/runs/e289c4c7ff47/years.jsonl'
```

**这个判红是对的**：脚本把产生清单的那台机器的临时目录写死成了默认数据源，
交付到别处就不能复核 —— 那就不算完整可交付证据。原样复现与修后对照见
`r1-repro-and-fix.txt`（复现办法是把那个临时目录 `mv` 走，用完放回，没有删任何数据）。

这一轮**只改了证据与复核脚本，没有动模型与后台 Python**
（`exp07/verify7.py`、`observer/adapter.py`、`observer/*_test.py` 一个字没改，
也没有重跑 600 年那组大测试 —— 那边正由根独立执行）。改法：

- 数据源换成两处仓库内的东西：**已提交的真实 API 响应** + **按 `real-cases.json`
  里记着的参数就地重算整段历史**。参数从证据文件里读，脚本里不写死任何路径或数值。
- 先证明「重算 == 已提交响应」（四条运行、17 个被引用的年度记录逐字段相同，
  引擎 sha256 也对得上），再用重算的整段历史承载需要全年数据的断言。
  **没有删掉任何一条一致性断言**：301 年逐年累计、6969 条事件去重、C 与 B 的
  301 年逐年对照、弃耕轨迹全部保留，检查项还从 88 条涨到 123 条。
- 已提交的卷宗与关系网也补了承重：`sizes` / 血缘 / 每条边的 `kcal` 与 `event_ids`
  都要逐条对回整段历史。
- 跨实例回放那一组先断言"样例与 A 是同一组参数"，再比内容 —— 输入也可重现了。
- 新增可选的 `--data-dir`：指向真正落过盘的观察台数据目录时，额外断言
  "盘上的记录与重算逐年相同"（本机实测四条各 301 年，0 处不同）。

为什么不是把上百 MB 的逐年记录塞进仓库：A 一条 300 年的 `years.jsonl` 就有 8.4 MB，
600 年是 104 MB（见 `archive-size-measured.txt`）。**确定性重算是这个项目从 EXP-01
起就验收过的性质**，用它换体积不需要牺牲任何断言 —— 而且脚本自己先证明了这一点。
