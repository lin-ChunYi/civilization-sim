...
> **后续变更（2026-09-14，C_ANIME_SUPPORT_01）**：写前端案例清单时发现 `cells` 的逐格数
> 加不回 `farm.year`（"潜在产出全没人收"的格没有事件、于是被写成 0）。
> 引擎加了一份只读的逐格结算痕迹、适配层改从它取数，新增 R12 / R12b 两项检验。
> **这次改动只影响观察**：42 组扫描的 `state_hash` / `full_digest` / `params_fingerprint`
> 与本目录的 `scan.json` 逐字段相同。详见 `docs/evidence/anime-cases-20260914/README.md`。

## 3. 两件如实记下的代价（已进 `docs/TODO-BACKLOG.md` #17 / #18）

> **2026-09-14 更正**：下面两条的数字在第一版里写错了 —— 我把 600 年的实测值记到了
> 300 年那一栏。重新按 `ls -l` 量了同一组参数（seed 4242 / `farm_m=250`）的三条运行，
> 结果如下；原文的"1000 年左右撞到 64 MB"是未实测的外推，已删。

- EXP-07 的逐年记录明显更大，而且**增长快于线性**（条数随人口涨）：
  120 年 **1.5 MiB**、300 年 **7.7 MiB**、600 年 **104.3 MiB**；续演再复制一份就翻倍。
  512 MB 的数据目录上限仍会到顶（到顶时如实报 `quota_exceeded`，不丢数据）。
- 检查点随 `farm_log` 增大，而且**每年都要整份重写**，所以长跑的写入量是 O(n²)：
  120 年 **0.4 MiB**、300 年 **2.5 MiB**、600 年 **18.6 MiB** ——
  本轮那条 600 年的对照运行，后 100 年明显变慢。
  `MAX_BYTES=64 MB` 会在第几年撞到**没有实测**，不给推算数字。
