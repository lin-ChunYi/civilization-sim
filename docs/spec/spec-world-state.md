...
- **证据等级闸门**（`mandate-contradictions` F12 第 2 条）：在 `tick < 2000` 期间，只有 A/B 级证据支撑的机制生效；C/D 级机制的对外接口返回"不存在/无效应"，而不是返回一个看起来合理的近似值（`pseudo-simulation` C9）。
...
见 StructuredOutput 的 `open_problems`。
