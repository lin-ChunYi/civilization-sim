...
由 exp02/verify2.py（冻结基线 commit c5a1f18）复制并只加一个机制：
迁移的代价作用于人口而不是储存。

研究的问题：**在迁移策略固定时，新增迁移死亡代价如何改变不同信息条件下的
人口、缺粮与迁移结果。** 该代价适用于**所有**迁移，不是只惩罚误判。

exp01/ 与 exp02/ 都只读、不改。两条退化恒等式：
  MOVE_MORT_M=0 且 SIGMA_M=0  =>  逐步复现 EXP-01 基线（用 exp01 自己的哈希）
  MOVE_MORT_M=0 且 SIGMA_M=s  =>  逐步复现 EXP-02 该档（用 exp02 自己的哈希）
全整数运算，无浮点进入状态。
"""
...
def params_fingerprint(sigma_m: int = None, move_mort_m: int = None) -> str:
    """全部规则参数 + 地图形状 + SIGMA_M + MOVE_MORT_M 的指纹。"""
    g = globals()
...
def run(seed, years, poison="", suppress=None, snap_at=None, sigma_m=0, move_mort_m=0):
    st = make_world(seed, poison, sigma_m, move_mort_m); snap = None
