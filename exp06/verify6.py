...
def make_world(seed: int, poison: str = "", sigma_m: int = 0, move_mort_m: int = 0,
               share_m: int = 0, aid_m: int = 0, recip_m: int = 0):
...
def params_fingerprint(sigma_m: int = None, move_mort_m: int = None,
                       ...
                       recip_m: int = None) -> str:
    """全部规则参数 + 地图形状 + SIGMA_M + MOVE_MORT_M + SHARE_M + AID_M + RECIP_M 的指纹。"""
    g = globals()
...
def run(seed, years, poison="", suppress=None, snap_at=None, sigma_m=0, move_mort_m=0,
        share_m=0, aid_m=0, recip_m=0):
    st = make_world(seed, poison, sigma_m, move_mort_m, share_m, aid_m, recip_m); snap = None
