...
> **本节措辞的一处纠正（2026-09-10）**：**"我们本次未能获取"不等于"该资料不存在"，更不等于"作者未公开"。** 本轮的检索受多重限制（WebSearch 配额耗尽、付费墙、服务器拒连、非英文文献覆盖不足），因此下面每一条的准确表述都是**我们手上没有可引用的来源**，而不是对文献本身或作者的判断。凡下文出现"未找到""未获取"，都按这个意思读。

- **前现代国家财政占 GDP 的那组数字**（中国 3–4%、法国 9%、英国 12%、拿破仑战争期 18%）——本次检索未能获取其唯一来源（服务器全程拒连，我们无法独立核对标题与日期）。这组数字原本要用作财政护栏。**在拿到可引用来源之前不得使用**；但这不表示该来源不存在。
- **"农户管理的灌溉系统绩效优于机构管理"**——五条支撑文献全部未找到，该结论目前完全无来源支撑。
- **Turchin et al. 2013 的 12 个模型参数，本次未能获取**：论文正文未列出数值，而补充材料在期刊直链返回 403、PMC 返回 404。**403/404 是我们的取用失败，不是"作者未公开"的证据**——补充材料很可能存在，只是需要换途径（机构订阅、作者主页、CoMSES/Dataverse、直接联系作者）。这件事应作为一个待办的取用任务，不是一个关于该论文的结论。
  在拿到参数之前，我们只能自行标定。附带一个独立于此的已知代价：它的 R²=0.65 中有近 49 个百分点来自"军事技术源点在草原"这一外生设定（随机播种后掉到 0.17），内生化之后空间格局的不确定性必然大幅上升。
