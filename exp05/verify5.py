...
由 exp04/verify4.py（已审阅版本 commit 68015cc）复制并**只加一个机制**：
同一格内，有余粮的群体可以把一部分食物援助给当年缺粮的群体。

**这是一条明确设定的援助规则，不是涌现。** 本切片不宣称已经出现合作、道德或制度：
规则写死在这里，谁能给、给多少、给谁，全部由下面的公式决定。

相位位置：**采集结算（相位 2）之后、消费（相位 3）之前**。
...
exp01/ exp02/ exp03/ exp04/ 都只读、不改。退化恒等式：
  AID_M=0                            =>  逐步复现 EXP-04（用 exp04 自己的哈希）
  AID_M=0 且 SHARE_M=0               =>  逐步复现 EXP-03（用 exp03 自己的哈希）
全整数运算，无浮点进入状态。
"""
...
def make_world(seed: int, poison: str = "", sigma_m: int = 0, move_mort_m: int = 0,
    ...
    st = {'tick': 0, 'seed': seed, 'poison': poison, 'sigma_m': sigma_m,
          ...
          'aid_log': [],                      # (tick, 格, 供给方, 接收方, 数量 kcal)
...
def params_fingerprint(sigma_m: int = None, move_mort_m: int = None,
                       share_m: int = None, aid_m: int = None) -> str:
    """全部规则参数 + 地图形状 + SIGMA_M + MOVE_MORT_M + SHARE_M + AID_M 的指纹。"""
    g = globals()
...
def run_id(st) -> str:
    ...
    h.update(f"params={params_fingerprint(st['sigma_m'], st['move_mort_m'], st['share_m'], st['aid_m'])};".encode())
    return h.hexdigest()
...
def step(st, suppress_split_in=None):
    ...
    # --- 相位 2c aid：同格食物援助（EXP-05 唯一新增机制）---
    # 位置：采集结算之后、消费之前。可用粮食 avail = 当年采集 + 原有储粮。
    # 供给方先保留自己当年的需求 need，只有 avail − need 的部分可以援助；
    # 预算 = 那部分的 AID_M/1000（向下取整）。接收方最多补齐 need − avail 的缺口。
    # 没被接收的预算仍归供给方（本相位只搬运被真正接收的那部分）。
    #
    # **统一结算**：供给、缺口、分配全部按同一份"援助前状态"计算；
    # 分配与配对只依赖 band_id 升序，与群体遍历顺序无关；
    # 接收方不能在本相位内把刚收到的粮再转赠出去（它的预算按援助前状态算，是 0）。
    # size==0 的群体不参与（它当年没有需求，也马上会被移除）。
    aid_in = {bid: 0 for bid in st['bands']}
    aid_out = {bid: 0 for bid in st['bands']}
    st['aid_pre'] = {}          # 援助前状态快照：{bid: (avail, need, cell)}；不进哈希，供检验与展示
    am = st['aid_m']
    if am > 0:
        ...
        for c in sorted(groups):
            ...
            for bid in members:
                ...
                # 注入 aidoverbudget：仍按"有余粮才是供给方"分类，但预算按**全部可用粮食**算，
                # 于是供给方会动用自己当年的口粮（正常版只拿 avail − need 的那部分）。
                if sur > 0:
            ...
            S = sum(budget.values()); D = sum(gap.values())
            st['aid_supply'] += S; st['aid_demand'] += D
            T = min(S, D)
            if T <= 0: continue
            recv = largest_remainder(receivers, gap, T)
            give = largest_remainder(donors, budget, T)
            if 'aidovergap' in P and S > D:       # 注入：把预算全塞出去，接收方超过缺口
                base = {r: gap[r] * S // D for r in receivers}
                rem = S - sum(base.values())
                rank = sorted(receivers, key=lambda k: (-(gap[k] * S % D), k))
                for i in range(rem): base[rank[i % len(rank)]] += 1
                recv, give, T = base, dict(budget), S

            if 'aidseq' in P:                     # 注入：按遍历序边算边给，可被反复转赠
                live = dict(pre_avail)
                pairs = []
                for d in [x for x in order if x in members]:
                    sur = live[d] - pre_need[d]
                    bud = (sur * am // MILLE) if sur > 0 else 0
                    for r in [x for x in order if x in members and x != d]:
                        g = pre_need[r] - live[r]
                        if g <= 0 or bud <= 0: continue
                        amt = min(bud, g)
                        pairs.append((d, r, amt)); bud -= amt
                        live[d] -= amt; live[r] += amt
            else:
                # 配对：双方都按 band_id 升序做瀑布式匹配，结果与遍历顺序无关
                ...
                while di < len(ds) and ri < len(rs):
                    ...
                    pairs.append((d, r, amt))
                    dleft[d] -= amt; rleft[r] -= amt
                if 'aidregift' in P:              # 注入：刚收到的粮在本相位内再转赠
                    got = {r: rleft_amt for r, rleft_amt in
                           ((r, recv[r] - rleft.get(r, 0)) for r in rs)}
                    still = {r: gap[r] - (recv[r] - rleft.get(r, 0)) for r in rs}
                    for d in [x for x in rs if got.get(x, 0) > 0]:
                        bud = got[d]
                        for r in [x for x in rs if x != d and still.get(x, 0) > 0]:
                            amt = min(bud, still[r])
                            if amt <= 0: continue
                            pairs.append((d, r, amt)); bud -= amt; still[r] -= amt

            if not pairs: continue
            st['aid_events'] += 1                 # 一次多人援助活动 = 1
            st['aid_transfers'] += len(pairs)     # 逐笔转移 = len(pairs)
            st['aid_donors'] += len({d for d, _, _ in pairs})
            st['aid_receivers'] += len({r for _, r, _ in pairs})
            for d, r, amt in pairs:
                take = amt
                got = amt + 1 if 'aidfab' in P else amt   # 注入：凭空多出 1 kcal
                aid_out[d] += take
                aid_in[r] += got
                st['aid_kcal'] += take
                st['aid_log'].append((t, c, d, r, take))
...
MATERIAL_LEDGER_FIELDS = ('inflow', 'out_eat', 'out_spoil', 'out_move', 'out_lost',
                          'mig_total', 'mig_regret', 'next_ctr', 'aid_m',
...
def aid_ledger_error(st) -> int:
    """援助账恒等：逐笔记录的总量 == 账上的援助总量。必须恒为 0。
    （逐笔"供给方减少 == 接收方增加"由 step 内的同一个 amt 保证，
      再由能量守恒兜底：凭空生粮会让守恒误差非 0。）"""
    return sum(a for (_, _, _, _, a) in st['aid_log']) - st['aid_kcal']
...
def run(seed, years, poison="", suppress=None, snap_at=None, sigma_m=0, move_mort_m=0,
        share_m=0, aid_m=0):
    st = make_world(seed, poison, sigma_m, move_mort_m, share_m, aid_m); snap = None
